...
import (
	...
	"nholuongut/terraform-docs/terraform-config-inspect/tfconfig"
	"nholuongut/terraform-docs/terraform-docs/internal/reader"
	"nholuongut/terraform-docs/terraform-docs/internal/types"
...
func loadInputs(tfmodule *tfconfig.Module, config *print.Config) ([]*Input, []*Input, []*Input) {
	...
	for _, input := range tfmodule.Variables {
		...
		// convert CRLF to LF early on (https://nholuongut/terraform-docs/terraform-docs/issues/305)
...
func loadOutputs(tfmodule *tfconfig.Module, config *print.Config) ([]*Output, error) {
	...
	for _, o := range tfmodule.Outputs {
		...
		// convert CRLF to LF early on (https://nholuongut/terraform-docs/terraform-docs/issues/584)
